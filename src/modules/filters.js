...
const filter = (function(){
    ...
    const byPreset = function(array, date){
        ...
        if(date === "this week"){
            ...
            const endOfWeekDate = format(add(new Date, {
                days: 7,
            }), "yyyy-MM-dd").split("-")

            const beforeEndOfWeek = array.filter(task => {
                const taskDateArray = task.date.split("-")
                console.log(taskDateArray, endOfWeekDate)
                return isBefore(new Date(taskDateArray[0],taskDateArray[1],taskDateArray[2]), new Date(endOfWeekDate[0],endOfWeekDate[1],endOfWeekDate[2]))
            })
    ...
    const completedTasks = function(array){
        const completedTasksArray = array.filter(task => task.completed === true)
        return completedTasksArray.length
    }

    return { byProject, byPreset, completedTasks }
